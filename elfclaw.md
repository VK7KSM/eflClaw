...
2. **任务名加唯一约束**，同名任务的创建请求改为"更新"而不是返回 `already_exists` 空操作。
3. **一次性任务（`at`）无论成功失败都清理**，不再"失败后停用但留在库里"。
4. **新增"直接发文字"的提醒类型**：到期由代码直接推送到 Telegram，不经过 LLM，不会因为 429/503 而失败，也不会被误判为"已完成"从而消失。
5. `cron_list` 只输出精简字段（id/name/schedule/enabled/next_run），不把 `last_output`（最长 16KB）整段塞进去。
6. 时区默认悉尼，不再是裸 UTC。
7. Agent 类型的定时任务失败重试时，不能把已经执行过的工具（发消息、写文件、建任务）重跑一遍。

## 7. 记忆重新设计

1. **记事改成结构化记录**：内容 + 创建时间 + 到期时间（可空）+ 状态（未完成/已完成），不是自由格式的长 Markdown。
2. **每轮对话只注入"未完成"的记事**，每条都带日期，注入条数有上限——不再是"整份文件塞进 prompt"或"语义检索 top5"这两种极端。
3. **提醒就是带到期时间的记事**，到点由代码直接发送（见第 6 节第 4 条），不依赖 LLM 判断。
4. **关掉"每句聊天原文都自动存成记忆"**——这是记忆库被灌满对话噪音、把真正的记事挤出去的主因。聊天记录本来就有独立的日志系统，不需要再进记忆库。
5. embedding 调用失败时**照样把原文存下来**（不算向量），不能因为一次 429 就丢整条写入。
6. 中文全文检索启用 trigram 分词（SQLite FTS5 默认的 unicode61 分词器对中文基本不起作用）。
7. 系统提示词的"当前时间"**只保留一处实时注入**，消除"启动时烘焙一份、每条消息又追加一份"导致的两个时间源打架。

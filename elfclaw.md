...
### 16.1b 本地真实浏览器抓取（`local_browser = true`）

有些页面只有脚本跑完才有内容，有些站拒绝一切普通请求。这类源标 `local_browser = true`，由
`tools/local-browser`（本机已装的 Chrome + 持久化配置文件）抓取，用完即杀，不常驻。
资源实测：启动 0.36 秒，空载 484 MB，开 3 页峰值约 1 GB，杀掉后 2.8 秒归零。

- 机主可以用 `login` 模式在机器桌面上手动过验证或登录一次，会话存进配置文件供后续复用。
  K6 上通过交互式计划任务 `elfClaw_BrowserLogin` 触发（SSH 启动的窗口到不了控制台桌面）。
- 浏览器工具的反自动化识别部分由机主自行实现和维护（2026-09-25），代码在 cf-crawler 仓库
  `browser/index.mjs`，注释里有说明。

三个时段都能用这个方式：新闻（`fetch_source`）、会展和成人产业（`fetch_page`）。

普通网页（不带 `local_browser`）现在也先用 `parse_page_links` 直接解析：
按链接形态区分文章页和栏目页（`looks_like_article_path`：最后一段是三词以上的标题 slug，
或带 8 位以上数字 ID；栏目路径另有黑名单），标题要求 ≥20 字且 ≥4 个词。
解析不出条目才退回 cf-crawler 渲染。

### 16.2 成人产业推送（`kind = "adult"`，每天 14:00）

用户把成人产业当作正规行业研究（澳洲多州已去罪化或实行牌照制度）。推送分两部分：

- **行业动态**：没有标 `directory` 的来源走新闻流水线 `run_news`，提示词换成 `ADULT_SELECT_SYSTEM`。提示词说明这是受监管行业的研究简报、不是色情内容，要求客观专业、不说教、不复述露骨描写。分类为：法规执法、市场价格、经营管理、科技产品、从业者权益、亚洲动态、行业事件。
- **市场观察**：标了 `directory = true` 的广告板和评价区，由程序转成文字，模型调用一次，把每条广告或评价整理成记录，字段有地区、类型、价格、每小时价、币种、站点认证、评价要点和倾向、真实性判断及理由。
  - 代码核对每个字段是否在白名单内；链接取自页面，模型给的链接不用；
  - 代码再过滤一遍电话（8 位以上数字）、@账号、邮箱、网址、"微信: xxx" 这类联系方式；
  - 记录存 `state/adult_intel.db`（`listings` 表）。价格统计（每小时中位数、最低、最高，按地区、类型、币种分组）由代码计算；
  - 每次推送重写本地汇总 `workspace/intel/adult-industry.md`，内容有价格表、评价倾向、真实性存疑清单、风险信号和最近 50 条记录。
- **约定的边界**：
  - 不追查广告背后的真实身份，不存联系方式；
  - 有未成年、强迫、贩运迹象的内容不作为资源收录，只进 `flags` 表，并在推送里单独列出、附上举报建议。
- 情报源（2026-09-25 换过一轮）：
  - **华人从业者广告是主体**：百事通（bestong.com.au）悉尼、墨尔本、布里斯班 × 成人服务、私钟援交共 6 个分类，
    每个配 `profile_pattern` 抓详情页；详情页给出店名、区号、地址和店铺自己的官网。
  - 亚洲：香港 141go161、City Heaven 东京（`local_browser`）、PTT 性版。
  - 西人只作对比参考：RealBabes 悉尼/墨尔本、Scarlet Blue。
  - 行业动态只保留科技、金融支付、平台经营、新产品四类，**不要警方执法和政府法规**（爸爸明确说是垃圾信息）。
- 记录带 `ethnicity`（华人/亚裔/西人/其他）：价格统计按族裔分组，推送里华人亚裔在前、西人单列作对比。

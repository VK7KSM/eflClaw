...
0. **单分支开发（main-only）。** 所有开发工作直接在 `main` 分支上进行。**禁止新建任何功能分支、修复分支或开发分支。** 提交直接推送到 `main`，不使用 PR 流程。此规则优先级最高，覆盖本文档中所有关于分支、worktree、PR 流程的旧规定。
1. **始终使用中文，用户看不懂英文。** 所有面向用户的输出——聊天回复、解释、提问、git commit message、PR 描述、dev_log.md 条目、新建的说明文档（如 elfclaw.md）——一律使用中文。唯一例外：代码本身（标识符、注释、字符串字面量）按 §6.3 命名规范保持英文；技术专有名词（如 API、token、cron、commit）可保留英文原词嵌在中文句子里。**Commit message 的标题和正文说明必须是中文**，不要像 2026-09-23 那次一样写成整段英文。

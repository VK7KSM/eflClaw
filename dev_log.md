...
### 补充：13 万 token 的真正成因（2026-09-25 复盘）

分批调用只限制了单次调用的大小，没限制调用次数，所以先查清了页面到底从哪来。K6 的
`adult_intel.db` 里 `profiles_read` 有 **80 条**（百事通 60、RealBabes 10、Scarlet Blue 10）。

成因是两个数量相乘，而我只给其中一个设了上限：

1. 13 个 `directory` 源，每个页面文本上限 12000 字；
2. 其中 9 个配了 `profile_pattern`，**每个源还会额外抓最多 10 个资料页**（`MAX_PROFILES`），
   实际抓了 80 个。这些资料页通过 `pages.extend(profiles)` 并入同一个列表。

一次调用因此带了 **93 个页面**。我设了 `DIRECTORY_TEXT_CHARS`（单页上限）却没设总量上限，
而单页上限在 93 个页面面前没有意义。

本地没暴露是因为**失败掩盖了问题**：开发机上 RealBabes 两个源和 Scarlet Blue 都是 403，
这三个源一个资料页都没抓；更早一次连 141 也超时。**源抓得越成功，提示词越大**——
K6 上一切正常反而触发了它。

表现为"模型没整理出内容"而不是报错：13 万 token 让 3.5-flash 超限，降级到 flash-lite 后
小模型在超长输入下没按 JSON 输出，`resolve_extracted` 返回 None。
...
**用户指出显示太模糊**："剩约 420（已用 180）"里 420 是怎么来的、3.6-flash 只有"已用 2"没有总数、
其他模型根本不显示。重做成每个数字都能自解释：

```
🔑 今日用量（17:00 重置）
• 3.5-flash 180/约 180（6 个 key × 单 key 约 30），2 个 key 已用完
• 3.5-flash-lite 12、3.6-flash 1（还没有 key 触顶，上限未知）
• 今天没用到：3.7-flash、3.8-flash
```

- 已观测到上限的模型：`已用/约总量` 并写明总量怎么来的；
- 还没触顶的：明说"上限未知"，不给一个没有分母的数字；
- 配置里有但今天没调用的也列出来——否则分不清"没用到"和"没统计"。

**看门狗**（K6 计划任务 `elfClaw_Watchdog`，每 5 分钟）：发现 zeroclaw.exe 不在就
`schtasks /run` 拉起守护进程的任务（而不是直接起 exe，保证落在交互式会话里，有头浏览器需要）。
检测到 `stage` 目录存在时跳过，避免和部署抢。实测：杀掉进程后成功拉起。

**迟跑提醒**（`cron/scheduler.rs`）：`due_jobs` 用的是 `next_run <= now`，所以停机期间错过的任务
**会在下次轮询补跑**，不会被跳过——但补跑是静默的，一条"早报"在 11 点送达看不出任何异常。
现在超过 10 分钟的补跑会在推送开头加一行说明迟了多久。

### 一次严重的误判（记录下来避免重犯）

我一度报告"早报和会展今天没有运行、守护进程静默死亡"，并据此分析了 RDP 断连、
查了系统事件日志和崩溃记录。**全部是错的。**

原因：`elfclaw-logs.jsonl` 里的时间戳是 **UTC**（`+00:00`），我当成悉尼时间读了。
早报 07:00 悉尼 = 21:00 UTC 前一日，会展 09:00 = 23:00 UTC 前一日。我按 `2026-09-26` 过滤，
只捞到了 10:00 悉尼（00:00 UTC）之后的记录，于是把"10 小时前的正常记录"当成了"最后的日志"。

实际情况：早报 07:00:08 跑完用时 31 秒、会展 09:00:08 用时 46 秒、心跳 10:00:08 用时 44 秒，
**全部正常**。以后读这个日志一律先转成悉尼时间再判断。

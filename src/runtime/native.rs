...
/// Native runtime — full access, runs on Mac/Linux/Docker/Windows
...
// elfClaw: detect shell at runtime rather than hard-coding PowerShell.
// On Windows, Git Bash (bash) is preferred; fallback chain matches upstream.
...
{
    if is_windows {
        // Windows priority: prefer POSIX-compatible shells first, then PowerShell, then cmd
        for (name, kind) in [
            ("bash", ShellKind::Bash),
            ("sh", ShellKind::Sh),
            ("pwsh", ShellKind::Pwsh),
        ...
        // Final fallback: COMSPEC env var (usually cmd.exe)
...
impl NativeRuntime {
    ...
    /// Returns the name of the selected shell kind (e.g. "bash", "powershell").
    /// Used by doctor diagnostics to report which shell will be used.
    ...
    pub(crate) fn selected_shell_program(&self) -> Option<&Path> {
        self.shell.as_ref().map(|s| s.program.as_path())
    }
...
impl RuntimeAdapter for NativeRuntime {
    ...
    fn has_shell_access(&self) -> bool {
        // elfClaw: now accurately reports shell availability instead of always returning true
        self.shell.is_some()
    }
    ...
    fn build_shell_command(
        ...
        // elfClaw: normalize python3 → python on Windows where python3 is not available
        #[cfg(target_os = "windows")]
        let command_owned;
        #[cfg(target_os = "windows")]
        let command = if command.starts_with("python3 ") || command == "python3" {
            command_owned = command.replacen("python3", "python", 1);
            &command_owned
        } else {
            command
        };
...
mod tests {
    ...
    #[test]
    fn native_has_shell_access_reflects_actual_availability() {
        // has_shell_access() must match whether we actually detected a shell
        let runtime = NativeRuntime::new();
        assert_eq!(runtime.has_shell_access(), detect_native_shell().is_some());
    }

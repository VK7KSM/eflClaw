...
fn link_count(_metadata: &Metadata) -> u64 {
    // `number_of_links()` requires the `windows_by_handle` unstable feature.
    // On Windows, hard links are uncommon in practice; conservatively return 1
    // so workspace guards are not incorrectly triggered.
    1
}

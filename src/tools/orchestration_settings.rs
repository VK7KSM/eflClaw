...
"#,
    ...
    #[test]
    fn load_orchestration_settings_uses_defaults_when_sections_absent() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("config.toml");
        std::fs::write(
            &path,
...
"#,
    ...
    #[test]
    fn load_orchestration_settings_errors_on_missing_file() {
        let result = load_orchestration_settings(Path::new("/nonexistent/config.toml"));

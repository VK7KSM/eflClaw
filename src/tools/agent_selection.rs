...
fn selection_score(
    ...
    // elfClaw: provider/model are Option<String>, use as_deref().unwrap_or("")
    let provider_str = agent.provider.as_deref().unwrap_or("");
    let model_str = agent.model.as_deref().unwrap_or("");

    let mut metadata = String::new();
    metadata.push_str(name);
    metadata.push(' ');
    metadata.push_str(provider_str);
    metadata.push(' ');
    metadata.push_str(model_str);
    metadata.push(' ');
    ...
    let provider_lc = provider_str.to_ascii_lowercase();
    let model_lc = model_str.to_ascii_lowercase();
...
mod tests {
    ...
    #[test]
    fn empty_agents_returns_error() {
        let agents: HashMap<String, DelegateAgentConfig> = HashMap::new();
        let err = select_agent(&agents, None, "test", "", true, None).unwrap_err();
        assert!(err.to_string().contains("No delegate agents"));
    }
}

...
fn build_ws_system_prompt(
    ...
    model: &str,
    tools_registry: &[crate::tools::ToolSpec],
    native_tools: bool,
) -> String {
    let mut tool_specs = tools_registry.to_vec();
    ...
    // Append tool-use protocol instructions when not using native tool calling.
    // We use a lightweight inline builder here because we only have ToolSpec (not Box<dyn Tool>).
    if !native_tools && !tool_specs.is_empty() {
        use std::fmt::Write;
...
{\"name\":\"tool_name\",\"arguments\":{}}\n\
        ...
        tool_block.push_str("You may use multiple tool calls in a single response. ");
        tool_block.push_str("After tool execution, results appear in <tool_result> tags. ");
        tool_block
...
async fn handle_socket(mut socket: WebSocket, state: AppState) {
    ...
    while let Some(msg) = socket.recv().await {
        ...
        match super::run_gateway_chat_with_tools(&state, &content).await {
            Ok(response) => {
                // Use empty tool slice for sanitization — we only have ToolSpec here,
                // not Box<dyn Tool>. Sanitization still strips bare tool-call XML blocks.
                let safe_response = finalize_ws_response(&response, &history, &[]);
...
Reminder set successfully."#;
    ...
    fn build_ws_system_prompt_includes_tool_protocol_for_prompt_mode() {
        ...
        let tools: Vec<crate::tools::ToolSpec> = vec![crate::tools::ToolSpec {
            name: "schedule".to_string(),
            description: "Mock schedule tool".to_string(),
